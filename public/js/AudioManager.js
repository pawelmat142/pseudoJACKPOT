...
export class AudioManager {
    ...
    constructor (vol) {
        ...
        if (!mobile()) {
            this.interface = document.body.appendChild(getInterface())
            this.bar = document.getElementById('volume-bar')
            this.icon = document.getElementById('volume-icon')
    
            this.icon.addEventListener('click', event => this.mute(event))
            this.interface.addEventListener('mouseenter', event => this.enter(event))
            this.interface.addEventListener('mouseleave', event => this.leave(event))
        }
    ...
    enter = () => {
        if (this.interface.classList.contains('hide')) {
            this.interface.classList.toggle('hide')
            this.interface.classList.toggle('hide')
        }
        if (!this.interface.classList.contains('show')) {
    ...
    setBar = (percent) => this.bar.style.height = percent < 100 ? `${percent*100}%` : `${percent}%`
...
const getInterface = () => {
    ...
    bar.setAttribute('id', 'volume-bar')
